...
All notable changes to ZerFi are documented in this file.
...
- Added `zerfi` as a globally registered Termux command via `$PREFIX/bin`
- `zerfi` — run with default settings (wlan0 + Pixie Dust)
- `zerfi menu` — open interactive menu without triggering auto-attack
- `zerfi old` — run the legacy v1 engine (w1.py) for compatibility
- `zerfi update` — pull latest updates from GitHub and re-apply setup
- `zerfi help` — open the built-in interactive help guide
- `zerfi fix` — launch the root/superuser repair script
- `zerfi contact` — open the developer contact menu
...
- `zerfi update` internally re-runs `install.sh` to keep the command fresh

### Built-in Help Guide (`help.py`)

- Added `help.py` — a full interactive help guide with 9 sections
- Section 1: What is ZerFi — introduction and overview
...
- Section 9: Quick reference for all `zerfi` commands
